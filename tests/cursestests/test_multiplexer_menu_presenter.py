...
from src.tui.presenters import MultiplexerMenuPresenter
...
class TestConstructor:
    def test_given_valid_arguments_returns_instance_of_file_menu_presenter(
        ...
        assert isinstance(
            MultiplexerMenuPresenter(mock_view, mock_model), MultiplexerMenuPresenter
        )

    def test_implements_presenter_interface(self, mock_view, mock_model):
        assert isinstance(MultiplexerMenuPresenter(mock_view, mock_model), Presenter)


class TestToggleActive:
    def test_toggles_active_state(self, mock_view, mock_model):
        presenter = MultiplexerMenuPresenter(mock_view, mock_model)
...
class TestUpdateView:
    ...
    def setup(self, mocker, mock_view, mock_model, session_labels):
        ...
        self.presenter = MultiplexerMenuPresenter(self.view, self.model)
        self.model.list_running_sessions.return_value = self.session_labels

    def test_updates_sessions(self):
        self.model.list_running_sessions.return_value = []
    ...
    def test_annotates_saved_session(self):
        self.model.list_saved_sessions.return_value = [self.session_labels[1]]
        self.presenter.update_view()
        self.view.render.assert_called_once_with(
            ["1. session1", "2. session2 (saved)"], 0, False
        )

    def test_annotates_saved_and_active_session(self):
    ...
    def test_annotates_saved_session_and_different_active_session(self):
        self.model.get_active_session.return_value = self.session_labels[0]
        self.model.list_saved_sessions.return_value = [self.session_labels[1]]
        self.presenter.update_view()
        self.view.render.assert_called_once_with(
